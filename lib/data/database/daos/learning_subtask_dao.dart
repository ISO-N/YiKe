...
class LearningSubtaskDao {
  ...
  Future<List<LearningSubtask>> getByLearningItemIds(
  ...
  Future<int> deleteByLearningItemId(int learningItemId) {
    return (db.delete(
      db.learningSubtasks,
    )..where((t) => t.learningItemId.equals(learningItemId))).go();
  }

  /// 批量查询学习内容的子任务数量（用于首页/任务中心/日历摘要）。
  ///
  /// 返回值：Map（key=learningItemId，value=count）。
  Future<Map<int, int>> getCountsByLearningItemIds(
    List<int> learningItemIds,
  ) async {
      ...
      await db.batch((batch) {
        for (var i = 0; i < subtaskIds.length; i++) {
          ...
          batch.update(
            ...
            where: (t) =>
                t.id.equals(id) & t.learningItemId.equals(learningItemId),
          );
        }
      });
    });
